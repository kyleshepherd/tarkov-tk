...
module.exports = async (msg, args) => {
	if (args.length < 1) {
		...
		let stats = [];
		await firebase.firestore().collection('kills').where('serverId', '==', msg.guild.id).orderBy('date', 'desc').get()
			.then((statsQuery) => {
		...
		if (msg.mentions.users.size < 1) {
			...
			let stats = [];
			await firebase.firestore().collection('kills').where('serverId', '==', msg.guild.id).where('killer', '==', player.id).orderBy('date', 'desc').get()
				.then((statsQuery) => {
					stats = statsQuery.docs.map(kill => ({
						date: kill.data().date,
						victim: kill.data().victim,
